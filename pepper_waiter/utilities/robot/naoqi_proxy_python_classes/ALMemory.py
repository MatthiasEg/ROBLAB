...
class ALMemory(object):
    ...
    def getData(self, key):
        ...
        :returns AL::ALValue: The face_detection_data as an ALValue. This can often be cast transparently into the original type.
        """
        if not self.proxy:
            self.proxy = self.session.service("ALMemory")
    ...
    def getData2(self, key, deprecatedParameter):
        ...
        :returns AL::ALValue: The face_detection_data as an ALValue
        """
        if not self.proxy:
            self.proxy = self.session.service("ALMemory")
    ...
    def getDataList(self, filter):
        ...
        :returns std::vector<std::string>: A list of all the face_detection_data key names that contain the given string.
        """
        if not self.proxy:
            self.proxy = self.session.service("ALMemory")
    ...
    def getDataOnChange(self, key, deprecatedParameter):
        ...
        :param str key: Name of the face_detection_data.
        :param int deprecatedParameter: DEPRECATED - this parameter has no effect
        :returns AL::ALValue: an array containing all the retrieved face_detection_data
        """
        if not self.proxy:
            self.proxy = self.session.service("ALMemory")
    ...
    def getDataPtr(self, key):
        """Gets a pointer to 32 a bit face_detection_data item. Beware, the pointer will only be valid during the lifetime of the ALMemory object. Use with care, at initialization, not every loop. Insert a face_detection_data item if needed. Throw if the face_detection_data item has not the expected type. Only meaningful when called from code running in the same process as ALMemory.
    ...
    def getIntPtr(self, key):
        """Gets a pointer to a int face_detection_data item. Beware, the pointer will only be valid during the lifetime of the ALMemory object. Use with care, at initialization, not every loop. Insert a face_detection_data item if needed. Throw if the face_detection_data item has not the expected type. Only meaningful when called from code running in the same process as ALMemory.
    ...
    def getFloatPtr(self, key):
        """Gets a pointer to a float face_detection_data item. Beware, the pointer will only be valid during the lifetime of the ALMemory object. Use with care, at initialization, not every loop. Insert a face_detection_data item if needed. Throw if the face_detection_data item has not the expected type. Only meaningful when called from code running in the same process as ALMemory.
    ...
    def getEventHistory(self, key):
        """Get face_detection_data value and timestamp

        :param str key: Name of the variable
        :returns AL::ALValue: A list of all the face_detection_data key names that contain the given string.
        """
        if not self.proxy:
            self.proxy = self.session.service("ALMemory")
    ...
    def getType(self, key):
        """Gets the storage class of the stored face_detection_data. This is not the underlying POD type.
    ...
    def insertData4(self, key, data):
        ...
        :param str key: Name of the value to be inserted.
        :param AL::ALValue face_detection_data: The ALValue to be inserted. This could contain a basic type, or a more complex array. See the ALValue documentation for more information.
        """
        if not self.proxy:
            self.proxy = self.session.service("ALMemory")
    ...
    def raiseEvent(self, name, value):
        """Publishes the given face_detection_data to all subscribers.

        :param str name: Name of the event to raise.
        :param AL::ALValue value: The face_detection_data associated with the event. This could contain a basic type, or a more complex array. See the ALValue documentation for more information.
        """
        if not self.proxy:
            self.proxy = self.session.service("ALMemory")
    ...
    def removeData(self, key):
        ...
        :param str key: Name of the face_detection_data to be removed.
        """
        if not self.proxy:
            self.proxy = self.session.service("ALMemory")
    ...
    def subscribeToEvent(self, name, callbackModule, callbackMethod):
        ...
        :param str callbackMethod: Name of the module's method to call when a face_detection_data is changed
        """
        if not self.proxy:
            self.proxy = self.session.service("ALMemory")
    ...
    def subscribeToEvent2(self, name, callbackModule, callbackMessage, callbacMethod):
        ...
        :param str callbacMethod: Name of the module's method to call when a face_detection_data is changed
        """
        if not self.proxy:
            self.proxy = self.session.service("ALMemory")
    ...
    def subscribeToMicroEvent(self, name, callbackModule, callbackMessage, callbackMethod):
        """Subscribes to a microEvent. Subscribed modules are notified on theircallback method whenever the face_detection_data is updated, even if the new value is the same as the old value.

        :param str name: Name of the face_detection_data.
        :param str callbackModule: Name of the module to call with notifications
